    ...
    def load(self, imageName, renderingSystem):
        ...
        vertexShader = """
            ...
            void main() 
            {
                ...
                //for improvement texture terrain Map to be normalized accurately
                norm = v2f_worldSpacePosition.xy * xyNormScale + 0.502;
                //for finding the terrain steepness
                worldSpaceNormal = normalize(normalIn);
                
                //distance to find the fog
                distance = -v2f_viewSpacePosition.z;
                //to get the camera world space coords
                cameraPosSpace = vec3(worldToViewTransform[3][0], worldToViewTransform[3][1],worldToViewTransform[3][2]);
                viewToVertPos = normalize(positionIn - cameraPosSpace);
...
"""
        ...
        fragmentShader = """
            ...
            uniform sampler2D highRockTexture;
            uniform sampler2D lowRockTexture;
            uniform sampler2D terrainMapTexture;
            uniform sampler2D pavemenTexture;

            vec3 terrainDataSample;
            vec2 v2f_texCoord;
            ...
            void main() 
            {
                ...
                v2f_texCoord = vec2(v2f_worldSpacePosition[0], v2f_worldSpacePosition[1]);
                vec3 grasSample = texture(grassTexture, v2f_texCoord * terrainTextureXyScale).xyz;

                vec3 terrainMapSample = texture(terrainMapTexture, norm).xyz;
                
                vec3 pavementSample = texture(pavemenTexture, v2f_texCoord * terrainTextureXyScale).xyz;
                vec3 highRockSample = texture(highRockTexture, v2f_texCoord * terrainTextureXyScale).xyz;
                vec3 lowRockSample = texture(lowRockTexture, v2f_texCoord * terrainTextureXyScale).xyz;
                //threshold of the steepness
                float threshold = 0.6;
                //steep calculation
                float steepTex = acos(dot(worldSpaceNormal, vec3(0,0,1)));
                terrainDataSample = mix(terrainMapSample, grasSample, 1);

                if (terrainMapSample[2] == 1)
                {
                    terrainDataSample = mix(grasSample, pavementSample, 1);
                    if (steepTex > 0.7)
                    {
                        terrainDataSample = mix(pavementSample, lowRockSample, 1);
                    }

                }

                else if (steepTex > threshold)
                {
                    terrainDataSample = mix(grasSample, lowRockSample, 0.75);
                }

                else if (v2f_height >= 70)
                {
                    terrainDataSample = mix(terrainDataSample, highRockSample, 1);
                }
                vec3 reflectedLight = computeShading(terrainDataSample, v2f_viewSpacePosition, v2f_viewSpaceNormal, viewSpaceLightPosition, sunLightColour);
                //+ headLight(terrainDataSample, v2f_viewSpacePosition, v2f_viewSpaceNormal, viewSpaceLightPosition, headlightColour);
                //vec3 totalLight = reflectedLight 
                //fragmentColor = vec4(toSrgb(reflectedLight), 1.0);
	        fragmentColor = vec4(toSrgb(applyFog(reflectedLight, distance, cameraPosSpace, viewToVertPos)), 1.0);
	        //fragmentColor = vec4(terrainMapSample, 1.0);
...
"""
        ...
        self.grass = ObjModel.loadTexture("grass2.png", "data", True)
        self.highRock = ObjModel.loadTexture("rock 2.png", "data", True)
        self.lowRock = ObjModel.loadTexture("rock 5.png", "data", True)
        self.pavement = ObjModel.loadTexture("paving 5.png", "data", True)
        self.terrainMap = ObjModel.loadTexture("track_01.png", "data", False)
