...
class RenderingSystem:
    ...
    //headLight
    uniform vec3 viewSpaceHeadLight;
    ...
    vec3 specularShading(vec3 materialColour, vec3 viewSpacePosition, \
            vec3 viewSpaceNormal, vec3 viewSpaceLightPos, vec3 lightColour)
    {
    ...
    vec3 computeShading(vec3 materialColour, vec3 viewSpacePosition, \
            vec3 viewSpaceNormal, vec3 viewSpaceLightPos, vec3 lightColour)
    {
        // TODO 1.5: Here's where code to compute shading would be placed most conveniently
        vec3 viewSpaceDirToLight = normalize(viewSpaceLightPosition - viewSpacePosition);
        //vec3 viewSpaceNormal = normalize(viewSpaceNormal);
        float incomingIntensity = max(0.0, dot(viewSpaceNormal, viewSpaceDirToLight));
        vec3 incommingLight = incomingIntensity * sunLightColour;

        vec3 outgoingLight = (incommingLight + globalAmbientLight) * materialColour;
        return outgoingLight;
    }

    vec3 applyFog ( vec3 materialColour, float distance, vec3 rayOri, vec3 rayDir)
    {
        //the FALLOFF density of the fog colour
        float b = 0.001;
        // c = a/b (opacity traverse the atmosphere)
        float c = 1;
        ...
        return mix(materialColour, fogColour, fogAmount);
    }

    vec3 headLight (vec3 materialColour, vec3 viewSpacePosition, vec3 viewSpaceNormal, vec3 viewSpaceLightPos, vec3 lightColour)
    {
        vec3 viewSpaceDirToLight = normalize(viewSpaceHeadLight - viewSpacePosition);
        float headlightIntensity = max(0.0, dot(viewSpaceNormal, viewSpaceDirToLight));
        vec3 incommingLight = headlightIntensity * headlightColour;

        vec3 outgoingLight = incommingLight * materialColour;
        return outgoingLight;
    }
    """
    objModelShader = None
    ...
    def setCommonUniforms(self, shader, view, modelToWorldTransform):
        ...
        # Head Light
        viewSpaceHeadLight = lu.transformPoint(view.worldToViewTransform, g_lightPos);
    ...
    def setupObjModelShader(self):
        self.objModelShader = lu.buildShader(["""
                ...
                void main() 
                {
                    ...
                    vec3 reflectedLight = computeShading(materialDiffuse, v2f_viewSpacePosition, v2f_viewSpaceNormal, viewSpaceLightPosition, sunLightColour)\
...
def update(dt, keyStateMap, mouseDelta):
    ...
    g_viewPosition = vec3(g_racer.position[0], g_racer.position[1], g_racer.position[2] + g_followCamOffset) - g_racer.heading * (g_followCamOffset)
...
#g_terrain.load("data/trees/birch_01_d.obj", g_renderingSystem);

g_racer = Racer()
g_racer.load("data/racer_02.obj", g_terrain, g_renderingSystem);

#g_props = Props()
for i in random.sample(g_terrain.treeLocations, 5):
        trees = Props()
        trees.load("data/trees/birch_01_d.obj", \
                   g_terrain, i, g_renderingSystem)
        g_props.append(trees)
for i in random.sample(g_terrain.rockLocations, 5):
        rocks = Props()
        rocks.load("data/rocks/rock_01.obj", g_terrain, i, g_renderingSystem)
        g_props.append(rocks)
